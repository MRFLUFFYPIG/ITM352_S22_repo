...
app.post("/process_form", function(request, response) {
    ...
    let quantity = request.body['quantity_textbox'];//access quantities entered from order form
    ...
    for (i in products_array) {
        ...
        if (isNonNegInt(quantity[i]) == false) {
            errors['invalid_quantity'[i]] = `${quantity[i]} is not a valid quantity for ${products_array[i].name} \n`;
        }

        //check if there is enough in inventory
        //access quantity_available from json file
        let inventory = products_array[i].quantity_available;
        ...
        if (Number(quantity[i]) <= inventory) {
            console.log(`${products_array[i].quantity_available} is new inventory amount`);
        }
        //if there's not enough in inventory, add error (quantity too large)
        if (quantity[i] > inventory) {
            errors[`invalid_quantity`[i]] = `Please order a smaller amount of ${products_array[i].name}! \n`;
        }
    ...
    if (Object.keys(errors).length === 0) {
        for(i in products_array){
            products_array[i].quantity_available -= Number(quantity[i]);
        }
